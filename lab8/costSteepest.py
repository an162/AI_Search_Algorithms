...
import random
def get_best_neighbor(state, travel_problem):
    ...
    for neighbor in neighbors:
        if neighbor.cost < best_neighbor_value:
            best_neighbor_state = neighbor
            best_neighbor_value = neighbor.cost
...
def stochastic_hill_climbing(travel_problem):
    ...
    current_state_cost = travel_problem.cost
    path=[]
    while True:
        ...
        if neighbor_state.cost <= current_state_cost:
...
def local_search(travel_problem, algorithm='steepest'):
    ...
    initial_node = Node(travel_problem.state)
    current_state = initial_node.state
    current_state_cost=initial_node.cost
    while True:
        
        if algorithm == 'steepest':
            neighbor_state = get_best_neighbor(current_state, travel_problem)
       ...
       #     return current_state
        ...
        if neighbor_state.cost <= current_state_cost:
            ...
            current_state = neighbor_state.state  
            current_state_cost=neighbor_state.cost
    return current_state
